    ...
    async def _get_prompt(self, device, username, password):
        async with asyncssh.connect(device, username=username,
            ...
            if 'Cisco' in server_version:
                ...
                async with conn.create_process(request_pty='force',
                                            term_type="vt100") as process:
                    process.stdin.write("\n")
                    process.stdin.write("#EOF#\r")
                    while True:
                        ...
                        if line:
                            ...
                            if "#EOF" in line.strip():
                                # print(line.strip())
                                prompt = line.strip()[:-len('#EOF#')]
                                if DEBUG: print(f'Prompt is: {prompt}')
                                break
...
def get_work(paramfile, devicegroup, workparams, devicecreds):
...
def assemble_influx_lp(measurements):
    ...
    if DEBUG: print('DEBUG: Measurements to process:\n{measurements}')
    influxlines = ""
    ...
    for device_results in aggregate_output:
        for output in device_results:
            ...
            if parsespec["matchtype"] == 'single':
                ...
                x = re.search(fr'{parsespec["regex"]}', output[3], re.S | re.M)
                if x:
                ...
                if DEBUG: pprint.pprint(parsespec)
                measurement = [output[0], parsespec["measurement"]]
                if statictags:
                    if DEBUG: print(f'Static tags are: {statictags}')
                    for statictag in statictags:
...
def get_run_specs(args):
    ...
    if DEBUG: print(deviceparams)

    worklist = get_work(args.paramfile, args.group, inventory, 
                        deviceparams)
    if DEBUG: print(worklist)
    ...
    if DEBUG: print(f'DEBUG: Pattern Matching Specs are:\n{parse_specs}\n===')
